...
#add user to weston-launch and video group
#if the user is not already part of.
function add_user_group() {
    group_update_done=0
    if [[ $AUTO_GROUP_ADD == 1 ]]; then
        for group in weston-launch video
        do
...
start_weston() {
    ...
    if [ "${result}" -ne "0" ]; then
        if add_user_group; then
            ...
            weston-launch &> /tmp/weston-startup.log &
            sleep 2
